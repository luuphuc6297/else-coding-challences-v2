...
} from '@infras/auth/interfaces/auth.interface'
import { IAuthService } from '@infras/auth/interfaces/auth.service.interface'
import { HelperDateService } from '@infras/helper/services/helper.date.service'
import { HelperEncryptionService } from '@infras/helper/services/helper.encryption.service'
...
export class AuthService implements IAuthService {
    ...
    constructor(
        ...
        this.accessTokenSecretKey =
            this.configService.get<string>('auth.accessToken.secretKey') ?? ''
        this.accessTokenExpirationTime =
            this.configService.get<number>('auth.accessToken.expirationTime') ?? 0
        this.accessTokenNotBeforeExpirationTime =
            this.configService.get<number>('auth.accessToken.notBeforeExpirationTime') ?? 0
        this.accessTokenEncryptKey =
            this.configService.get<string>('auth.accessToken.encryptKey') ?? ''
        this.accessTokenEncryptIv =
            this.configService.get<string>('auth.accessToken.encryptIv') ?? ''

        this.refreshTokenSecretKey =
            this.configService.get<string>('auth.refreshToken.secretKey') ?? ''
        this.refreshTokenExpirationTime =
            this.configService.get<number>('auth.refreshToken.expirationTime') ?? 0
        this.refreshTokenNotBeforeExpirationTime =
            this.configService.get<number>('auth.refreshToken.notBeforeExpirationTime') ?? 0
        this.refreshTokenEncryptKey =
            this.configService.get<string>('auth.refreshToken.encryptKey') ?? ''
        this.refreshTokenEncryptIv =
            this.configService.get<string>('auth.refreshToken.encryptIv') ?? ''

        this.payloadEncryption = this.configService.get<boolean>('auth.payloadEncryption') ?? false
        ...
        this.passwordExpiredIn = this.configService.get<number>('auth.password.expiredIn') ?? 0
        this.passwordSaltLength = this.configService.get<number>('auth.password.saltLength') ?? 0
    }
