...
import {IMessageService} from '@infras/message/interfaces/message.service.interface'
...
export class MessageService implements IMessageService {
  ...
  constructor(
    ...
    this.appDefaultLanguage = this.configService.get<string>('message.language') ?? 'en'
    this.appDefaultAvailableLanguage = this.configService.get<string[]>('message.availableLanguage') ?? ['en']
  }
  ...
  filterLanguage(customLanguages: string[]): string[] {
    return intersection(customLanguages, this.appDefaultAvailableLanguage)
  }
  ...
  get<T = string>(key: string, options?: IMessageOptions): T {
    const customLanguages =
      options?.customLanguages && options.customLanguages.length > 0
