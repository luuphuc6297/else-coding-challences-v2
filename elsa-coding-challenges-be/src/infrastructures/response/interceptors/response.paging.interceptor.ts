...
import { IResponsePaging } from '@infras/response/interfaces/response.interface'
import {
    ResponsePagingCursorMetadataSerialization,
    ResponsePagingMetadataSerialization,
    ResponsePagingSerialization,
} from '@infras/response/serializations/response.paging.serialization'
        ...
        if (context.getType() === 'http') {
            return next.handle().pipe(
                map(async (res: Promise<IResponsePaging>) => {
                    ...
                    const __pagination = request.__pagination ?? { perPage: 20, page: 1 }
